...
export const LizardCanvas: React.FC<VideoTextEngineProps> = ({
  prepared,
  fontSize,
  lineHeight,
  videoUrl,
  ...
  useEffect(() => {
    workerRef.current = new Worker(
      new URL("./chroma.worker.ts", import.meta.url),
      { type: "module" },
    );
    ...
    const startEngine = () => {
      ...
      const render = () => {
        ...
        if (glCanvas.width !== dw * dpr) {
          glCanvas.width = dw * dpr;
          glCanvas.height = dh * dpr;
        ...
        if (!isWorkerBusy && video.readyState >= 2) {
          ...
          isWorkerBusy = true;
          workerRef.current?.postMessage({ imageData, SW, SH }, [
        ...
        if (prepared && prepared.segments && currentProfile.length > 0) {
          ...
          while (
            ...
            const endScanY = Math.ceil(
              ((curY + lineHeight - rY) / rH + 0.01) * SH,
            );
            ...
            if (occupiedSpans.length > 0) {
              ...
              for (let i = 1; i < occupiedSpans.length; i++) {
                if (occupiedSpans[i][0] <= current[1] + 10)
                  current[1] = Math.max(current[1], occupiedSpans[i][1]);
                else {
                  merged.push([current[0], current[1]]);
                  current = [occupiedSpans[i][0], occupiedSpans[i][1]];
                }
              }
              merged.push(current);
            }
