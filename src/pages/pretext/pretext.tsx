...
const TEXT_CONTENT =
  `文字流通过 Pretext 引擎实时计算坐标，并使用 React 映射为原生 DOM 节点。这意味着文字拥有 100% 的矢量清晰度，完全不受 Canvas 像素操作的影响。影像层使用 WebGL 硬件加速抠像，逻辑层使用分布式 WebWorker 进行多槽位避障分析。这实现了文字钻进人物空隙、绕过不规则肢体的电影级视觉效果。所有的排版参数已针对当前视觉效果进行微调与固定。 `.repeat(
    25,
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        ...
        <LizardControls videoUrl={videoUrl} onVideoUpload={handleVideoUpload} />
      </div>
    </div>
  );
};
